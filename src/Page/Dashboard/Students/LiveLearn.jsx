...
const LiveLearn = () => {
    ...
    const fetchPaidExam = async () => {
      try {
        const res = await axios.get(
          `https://congozi-backend-6jc5.onrender.com/api/v1/purchases/access/${examCode}`,
          {
  ...
  useEffect(() => {
    const fetchExamDetails = async () => {
      try {
        ...
        if (!examId) return;
        const res = await axios.get(
          `https://congozi-backend-6jc5.onrender.com/api/v1/exams/${examId}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
  ...
  const fetchgGukoraExam = useCallback(async () => {
    try {
      ...
      const purchaseRes = await axios.get(
        `https://congozi-backend-6jc5.onrender.com/api/v1/purchases/user`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
      ...
      const res = await axios.get(
        `https://congozi-backend-6jc5.onrender.com/api/v1/exams/gukora/${number}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const handleShowPaymentPopup = async () => {
    ...
    try {
      ...
      const res = await axios.get(
        `https://congozi-backend-6jc5.onrender.com/api/v1/exams/kora/${paidExam.number}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const handlePayLaterClick = async () => {
    try {
      await axios.post(
        `https://congozi-backend-6jc5.onrender.com/api/v1/purchases/${gukoraExam._id}`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
