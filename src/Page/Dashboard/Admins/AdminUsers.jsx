...
const AdminUsers = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const response = await axios.get(
        `https://congozi-backend-6jc5.onrender.com/api/v1/users`,
        {
          headers: {
  ...
  const handleSaveUserEdit = async () => {
    ...
    try {
      ...
      setError(null);
      const payload = {};
      if (editedCompanyName !== userToEdit.companyName)
        payload.companyName = editedCompanyName || null;
      ...
      if (editedIdcard !== userToEdit.idCard)
        payload.idCard = editedIdcard || null;
      if (editedRole !== userToEdit.role) payload.role = editedRole || null;
      if (editedAddress !== userToEdit.address)
        payload.address = editedAddress || null;
      if (Object.keys(payload).length > 0) {
        await axios.put(
          `https://congozi-backend-6jc5.onrender.com/api/v1/users/${userToEdit._id}`,
          payload,
          {
            headers: {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      await axios.delete(
        `https://congozi-backend-6jc5.onrender.com/api/v1/users/${userToDelete._id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
