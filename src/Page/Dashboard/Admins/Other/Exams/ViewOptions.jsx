...
const ViewOptions = ({ question, onBack }) => {
  ...
  const fetchOptions = async () => {
    try {
      const res = await axios.get(
        `https://congozi-backend-6jc5.onrender.com/api/v1/options/${question._id}`
      );
  ...
  const handleSaveEditedOption = async () => {
    ...
    try {
      ...
      await axios.put(
        `https://congozi-backend-6jc5.onrender.com/api/v1/options/${optionToEdit._id}`,
        {
  ...
  const handleDeleteOption = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.delete(
        `https://congozi-backend-6jc5.onrender.com/api/v1/options/${optionToDelete._id}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
