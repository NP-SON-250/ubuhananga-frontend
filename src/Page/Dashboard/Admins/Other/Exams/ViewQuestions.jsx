...
const ViewQuestions = ({ exam, onBack }) => {
  ...
  const fetchQuestions = async () => {
    try {
      const res = await axios.get(
        `https://congozi-backend-6jc5.onrender.com/api/v1/questions/${exam._id}`
      );
  ...
  const handleSaveQuestionEdit = async () => {
    ...
    try {
      ...
      await axios.put(
        `https://congozi-backend-6jc5.onrender.com/api/v1/questions/${questionToEdit._id}`,
        formData,
        {
          headers: {
  ...
  const handleDeleteQuestion = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.delete(
        `https://congozi-backend-6jc5.onrender.com/api/v1/questions/${questionToDelete._id}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
